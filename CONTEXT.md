...
| Prism manifest | Canonical configuration manifest. Schema versioned (`setup_version` field, currently v6 per ADR-0043 and ADR-0049). Stores identity, scaffolding, model, variant, experimental flag, optional integration-key (`env`) configuration, and optional Boolean integration-preference fields (`mcp.*`, `plugins.*`) for MCP servers and npm plugins. Exists at two locations (see *project Prism manifest* and *user Prism manifest*). Parsed by a single dependency-free PHP JSONC reader (`PrismJsoncDocument` + `PrismManifest` CLI); no longer `jq`-sourced. Full JSONC (line/trailing `//` + block `/* */` + trailing commas). See ADR-0043 (supersedes ADR-0029, ADR-0032) and ADR-0045. |
