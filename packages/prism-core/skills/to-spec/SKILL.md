...
- The design has emerged from a `from-issue` or exploratory conversation and
  you need to record it as a spec.
...
- You are following the `consult` workflow — it cannot write `docs/specs/`;
  stop that workflow and load `brainstorming` for the transition.
- The design is still ambiguous — load `brainstorming`, which owns
  classifier-driven design and prototyping.
...
4. **Suggest `architect` for cross-cutting specs.** After finalizing the spec,
   ...
   recommend loading `architect` before ticketing/planning. It emits
...
- `architect` skill — load for cross-cutting specs before ticketing
...
- *Interviewing when you should synthesize* — if the design is unclear, you
  loaded the wrong skill. Load `brainstorming`, which owns design interviews
  through `grilling`.
