...
- A single, brainstormable new feature → load `brainstorming` (spec → branch
...
- **Research** (`wayfinder:research`, AFK): reading docs, third-party APIs, or
  local resources. Produces a markdown summary as a linked asset. Load the
  `websearch` or `searxng` skill when knowledge outside the working directory
  is required.
