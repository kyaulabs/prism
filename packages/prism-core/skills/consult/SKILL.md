---
name: consult
description: Use for conversational project exploration. Runs grilling, writes approved glossary terms to CONTEXT.md, proposes ADRs only when they meet the three-part test, and stops before the engineering pipeline. Does not write source code, specs, or plans.
---

Use this conversational project-exploration workflow to help users understand
their project — domain terms, architectural
...
The user's invocation message is their question or exploration goal. Engage
...
>
> - If this is a feature or a rough idea: load `brainstorming` to start the
>   brainstorming → spec → branch → plan → `tdd` pipeline.
> - If this is an existing issue: load `from-issue` with `#NN` to analyze,
>   plan, and route it.
>
...
- **Pipeline boundary — HARD.** While following this workflow, never load
  `brainstorming`, `writing-plans`, `executing-plans`, `tdd`, `architect`,
  `code-review`, or `debug`. This is an exploration-only zone. When the user
  wants implementation, stop this workflow and direct the transition.
...
- `AGENTS.md` — authoritative source for global boundaries and pipeline; the
  active adapter owns stack specifics
- `CONTEXT.md` — domain glossary, entities, invariants
- `adr/` — architecture decision records

## Gotchas

- *Crossing into implementation mid-conversation* — stop the consult workflow
  and direct the user to `brainstorming` or `from-issue`.
- *Writing an ADR for a routine choice* — require at least one part of the
  reversibility/foreclosure/cross-cutting test.
