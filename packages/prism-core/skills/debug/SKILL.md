...
repro tests in the active adapter's test roots, throwaway harnesses in
`prototypes/`, and temporary `[DEBUG-]`-tagged instrumentation. Get explicit
user approval before instrumenting production files. All scaffolding is
ephemeral — clean it up in Phase 6.
...
## Logs and stack commands

Load the active adapter's stack skill for production log locations, local
server output, focused test commands, syntax checks, and runtime diagnostics.
If no adapter is active, ask the user which stack applies rather than guessing.
Use only the minimum log slice needed for the symptom, and never expose
...
- Failing tests / regression tests → the active adapter's test roots.
- Throwaway harnesses / CLI scripts → `prototypes/` or `prototype_` prefix
  (matches the `prototype` skill convention).
- HTTP scripts → inline in your report and run only with any required external
  access approval.
...
**Instrumentation edits to production files require explicit user approval.**
Show exactly what you intend to instrument and let the user reject anything
that looks like a fix rather than a probe. Once approved, the `[DEBUG-]` tag
exists so Phase 6 cleanup finds it.

**Perf branch.** For performance regressions, logs are usually wrong. Instead:
establish a baseline measurement (timing harness, the active runtime's clock,
profiler, or query plan), then bisect. Measure first, fix second.
...
1. Turn the minimised repro into a failing test at that seam in the active
   adapter's test root.
2. Watch it fail.
...
**Log evidence:**
- <adapter-defined log path or source>: <relevant lines>
...
- You **may** write investigation scaffolding: repro tests in the active
  adapter's test roots, throwaway harnesses (`prototypes/` or `prototype_`
  prefix), and temporary `[DEBUG-]`-tagged instrumentation. Production-file
  instrumentation requires explicit user approval.
- All scaffolding must be removed in Phase 6 — a `[DEBUG-]` tag surviving
  into a commit is a defect.
- Prefer the active adapter's focused-test selector over running the full
  suite during the feedback loop.
