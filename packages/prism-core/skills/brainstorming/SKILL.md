...
- Documentation-only changes (README, comments, docblocks)
- RCS header / vim modeline additions or corrections
- Style/formatting-only changes with no logic impact (formatter or lint fixes)
...
- Implementation follows the normal single-agent pipeline (`writing-plans` →
  `executing-plans` + `tdd` → verification → `/check` → `code-review`); this
  brainstorming phase does not implement (ADR-0055).
...
The design cycle is complete. Load the `writing-plans` skill for
implementation planning. Do NOT begin implementation or load
`executing-plans`/`tdd` until the plan is approved.
...
- `writing-plans` skill — the implementation-planning phase that follows
  design; load it after the written spec is approved.
...
- `architect` skill — for non-trivial or cross-cutting changes, load it after
  the spec is written and before loading `writing-plans` (per AGENTS.md).
