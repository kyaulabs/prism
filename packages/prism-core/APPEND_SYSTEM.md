...
| "The coverage gate can slide" | The adapter's coverage gate (e.g. 80% line coverage on changed files) is measured, not approximated. No exceptions. |
...
vertical slice) precedes wayfinding via brainstorming (see ADR-0050).
...
For bugs, prepend the `debug` skill before the `tdd` skill on the fix.
