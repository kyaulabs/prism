...
8. **Finalize** — load `finishing-a-development-branch`; after artifact cleanup it pauses once, then an accepted attempt synchronizes, attests, runs `/check`, performs all four `code-review` axes, revalidates SHAs, and invokes preparation-only `/pr` automatically. The initial review starts a bounded review chain. After a Blocking repair, fresh acceptance reviews only the continuous repair delta. Advisory findings do not block `/pr`; base or history changes, discontinuity, incomplete axes, or mismatched HEAD invalidate the chain. Repairs require fresh finalization acceptance. Review-chain state remains untracked under `.pi/`; `/pr` creates neither issues nor pull requests. Humans own pushing and GitHub publication.
