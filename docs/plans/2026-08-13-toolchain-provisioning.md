...
- Mandatory external versions: `semgrep@1.173.0`, `@alibaba-group/open-code-review@1.9.2` (`ocr`). Prism runtime/setup verifies but never installs or configures them.
...
- `CONTEXT.md` already defines the Pi-era `toolchain contract`, scope entities, consent boundaries, and ADR-0061; implementation must preserve that vocabulary and requires no further domain-context change unless a new term appears.
...
test('rejects a version range and duplicate component id', () => {
	const invalid = {
		schemaVersion: 1,
...
```json
{
  ...
  "components": [
    ...
    {"id": "ocr", "kind": "command", "ecosystem": "npm", "package": "@alibaba-group/open-code-review", "version": "1.9.2", "provisioning": "external", "authentication": "required", "executable": "ocr", "versionArguments": ["--version"], "argumentPolicy": {"mode": "first-token", "allowed": ["review", "scan"]}}
  ]
}
```
...
`contract.js` must export the three named functions; reject unknown top-level/component keys, non-object values, unsupported schema/role/kind/ecosystem/provisioning/authentication/policy values, non-exact semver, absolute handler paths, duplicate IDs, command entries missing executable/version arguments/policy, library entries carrying executable fields, and adapter browser targets other than `chromium`. Freeze the returned object recursively.
...
- Produces: `checkExternalTools({contract, env, run})`, `testOcrConnectivity({approved, run})`, `doctor()`.
...
Use temporary fake `semgrep` and `ocr` executables. Cover exact pass, missing command, wrong version, conflicting version tokens, timeout, output cap, Semgrep without login, doctor without OCR-test approval, approved `ocr llm test`, failed live test, and canary-secret output. Assert:
...
Resolve external executables from `PATH` without invoking a shell. Run each declared `versionArguments`, extract one version token, and compare exact strings. Return checks with only tool ID, PASS/FAIL, expected, actual version when safe, and a fixed message. Never include raw stdout/stderr.
...
Expected for later integration: exactly `1.173.0` and `1.9.2`. If either differs, stop and give the human the pinned remediation; do not install it.

- [ ] **Step 5: Commit**

```bash
git add packages/prism-core/scripts/prism-tool/preflight.js packages/prism-core/scripts/prism-tool/cli.js tests/Node/prism-tool-preflight.test.js tests/Node/prism-tool-run.test.js
...
Update existing hook fixtures to set `PRISM_TOOL` to the fake or real source CLI and provide fake exact Semgrep/OCR executables. Replace assertions about local `node_modules/commitlint` and `npm install` remediation with launcher assertions, including the focused `commit_msg_parity_test.sh` contract.
...
Retain each prompt/skill's current behavioral constraints, frontmatter, and one-question gates. Do not duplicate exact version tables outside contracts; link to the contract and ADR-0061.
...
- Clarifies: exact Semgrep/OCR installation in ephemeral CI is environment provisioning; Prism runtime/setup remains verification-only.
...
- CI environment provisioning names Semgrep `1.173.0` and OCR `1.9.2` exactly;
...
Remove stale instructions that require root `node_modules`, direct `git cliff`, optional OCR, Pest 4, OpenCode, or package-source execution from consumer projects. Keep global `AGENTS.md` concise and point to ADR-0061/contracts rather than duplicating version tables.
...
- **Documentation/context coverage:** ADR-0061 and `CONTEXT.md` were accepted before this plan; Tasks 9, 10, and 12 align active package resources and user documentation.
