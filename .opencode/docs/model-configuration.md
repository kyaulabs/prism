...
1. `.opencode/setup.json` (models section) — committed defaults, extracted by
   direnv `.envrc` (via jq) on directory entry.
...
1. Edit `.opencode/setup.json` — change the model and/or variant value(s)
