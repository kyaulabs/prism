...
| `@from-issue` | subagent | Issue on-ramp — fetches an existing GitHub issue, classifies type, grills one-at-a-time, applies one Type + one Progress value, analyzes, plans, halts for approval, creates the branch, and hands off — the user invokes `@tdd` directly (its `ask`-gated prompts only render at depth 1); routes bugs to @debug and chores to the fast-path |
