...
- Model selection: all `model` fields in `opencode.json` and `.opencode/agents/*.md` use `{env:VAR}` substitution (e.g. `{env:OPENCODE_MODEL_PRIMARY}`) rather than hard-coded model IDs. Defaults ship in `.opencode/models.default.env`, sourced automatically via direnv `.envrc`. Use `/setup` to configure models per-tier. Primary agents that omit `model:` inherit the top-level `model` (which itself is `{env:VAR}` — resolved at runtime). Subagents keep explicit `model:` lines with `{env:VAR}` references for deterministic tier assignment. See ADR-0012.
